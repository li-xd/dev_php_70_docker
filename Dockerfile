...
apt-get update && \
apt-get install -y php7.0-fpm php7.0-curl php7.0-mysql php7.0-mcrypt php7.0-gd php7.0-zip php-memcached php-gearman php-mongodb php-redis php-mbstring php7.0-mbstring php7.0-xml php7.0-intl php-xml php-xdebug git && \
...
sed -E -i "s/^error_reporting\ .+?$/error_reporting = E_ALL \& \~E_DEPRECATED \& \~E_STRICT \& \~E_NOTICE/" /etc/php/7.0/fpm/php.ini && \
mkdir /var/run/php && \
mkdir /var/www && \
...
echo "xdebug.remote_port=5900" >> /etc/php/7.0/fpm/conf.d/20-xdebug.ini && \
echo "xdebug.remote_host=docker_host" >> /etc/php/7.0/fpm/conf.d/20-xdebug.ini
